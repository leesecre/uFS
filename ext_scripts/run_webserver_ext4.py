...
for num_app in range(1, 11):
    ...
    if readonly:
        filebench_command = [
        "env", f"NUM_WORKER={num_app}", "./record_cpu_util.py", f"{output_dir}/cpuutil_webserver{num_app}", f"./filebench -f workloads/webserver{num_app}-ro.f"]
    else:
        filebench_command = [
        "env", f"NUM_WORKER={num_app}", "./record_cpu_util.py", f"{output_dir}/cpuutil_webserver{num_app}", f"./filebench -f workloads/webserver{num_app}.f"]
