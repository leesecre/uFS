...
def start_filebench(num_app, num_worker, filebench_out):
    filebench_command = [
        "env", f"NUM_WORKER={num_worker}", "./record_cpu_util.py", f"{output_dir}/cpuutil_fileserver{num_app}_{num_worker}",  f"NUM_APP={num_app} ./filebench -f workloads/fileserver.f"
    ]
