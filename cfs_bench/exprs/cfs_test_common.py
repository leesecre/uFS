...
def use_exact_num_app():
    if "USE_EXACT_NUM_APP" in os.environ:
        if os.environ["USE_EXACT_NUM_APP"].lower() == "true":
            ...
            raise RuntimeError(
                'Invalid USE_EXACT_NUM_APP: Only accept "true" OR "false"'
            )
    return False
...
def use_single_worker():
    ...
    if use_single.lower() == "false":
        return False
    raise RuntimeError(
        'Invalid CFS_BENCH_USE_SINGLE_WORKER: Only accept "true" OR "false"'
    )
...
def start_fs_proc(
    ...
    if check_if_process_running(fs_main_bin.split("/")[-1]):
        print(
            "ERROR. FSP ({}) is already running. EXIT ...".format(
                fs_main_bin.split("/")[-1]
            )
        )
        sys.exit(1)
...
def start_mtfsp(num_fsp_worker, num_app_proc, ready_fname=None, bypass_exit_sync=False):
    fsp_shmkeys, app_key_dict = gen_expr_mtfsp_shm_offset(num_fsp_worker, num_app_proc)
    print(fsp_shmkeys)
    print(app_key_dict)
    fsp_do_clean_sock()
    ...
    p_fs, exit_signal_file_name, real_ready_fname, fs_cmd = start_fs_proc(
        num_fsp_worker,
        num_app_proc,
        shm_offset=fsp_shmkeys,
        ready_fname=ready_fname,
        bypass_exit_sync=bypass_exit_sync,
    )
...
def get_microbench_bin(is_fsp=True):
    if is_fsp:
        cfs_bin = "{}/{}".format(get_cfs_root_dir(), "cfs_bench/build/bins/cfs_bench")
    else:
        cfs_bin = "{}/{}".format(
            get_cfs_root_dir(), "cfs_bench/build/bins/cfs_bench_posix"
        )
    return cfs_bin
...
def get_coordinator_bin():
    return "{}/{}".format(
        get_cfs_root_dir(), "cfs_bench/build/bins/cfs_bench_coordinator"
    )


def get_offline_checkpointer_bin():
    return "{}/{}".format(
        get_cfs_root_dir(), "cfs/build/test/fsproc/fsProcOfflineCheckpointer"
    )
    ...
    while True:
        ...
        if fr_started and fw_started:
            if fwlines[fw_idx].strip() != frlines[fr_idx].strip():
                print("ERROR cannot match line:{} line:{}".format(fw_idx, fr_idx))
                return
            else:
                fw_idx += 1
                fr_idx += 1
