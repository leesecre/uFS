...
PERF_SCRIPT_NAME_PATTERN = re.compile(r"^perfthp_(\w+)_iosize(\d+)_(\d+)$")
WORKLOAD_RANK = {
    "append": 0,
    "seqwrite": 1,
    "rwrite": 2,
...
def collect_microbench_script_paths(batch_root: Path, filesystem_key: str):
    ...
    microbench_performance_parse_breakdown.collect_perf_files).
    """
    found = []
    ...
    for run_dir in batch_root.rglob(run_glob):
        ...
        for app_dir in run_dir.rglob(app_glob):
            for script_path in app_dir.rglob("perfthp_*"):
                ...
                name_match = PERF_SCRIPT_NAME_PATTERN.match(stem)
                if not name_match:
                    continue
                wl, iosize_str, app_str = name_match.groups()
                found.append(
                    (
...
def batch_parse_microbench_scripts(
    ...
    if fs_slug_local is None:
        print(
            f"ERROR: --batch only supports filesystem tree oxbow or ext4; "
    ...
    if not scanned:
        print(
            ...
            f"(expected: under .../log_{fs_slug_local}_*_throughput_app_*/**/perf/"
            f"perfthp_*{SCRIPT_SUFFIX}, same discovery as breakdown). "
            "Export scripts with microbench_performance_parse_breakdown.py first.",
            file=sys.stderr,
        )
        write_csv([], output_path)
...
def main():
    ...
    parser.add_argument(
        ...
        help="System represented by this perf script",
    )
    parser.add_argument(
        "--batch",
        action="store_true",
        help="Scan --batch-root for microbench perfthp_*.perf.script files",
    )
    parser.add_argument(
        "--batch-root",
        type=Path,
        default=Path("."),
        help="Root directory for --batch (default: current directory)",
    )
    parser.add_argument(
        "--output",
        "-o",
