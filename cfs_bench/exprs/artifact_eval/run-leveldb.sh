...
if [ ! "$2" = "ufs" ] && [ ! "$2" = "ext4" ] && [ ! "$2" = "oxbow" ] && [ ! "$2" = "omnicache" ]; then print_usage_and_exit; fi
...
function run_one_workload_omnicache() {
	workload=$1
	data_dir="$(mk-data-dir leveldb_${workload}_omnicache)"
	echo "Run LevelDB: $workload"
	sudo -E python3 scripts/run_ldb_omnicache.py "$workload" "$data_dir" "${@:2}"
}
...
if [ "$2" = "ufs" ]; then
	...
	if [ "$1" = "all" ]; then
		for job in 'a' 'b' 'c' 'd' 'e' 'f'
		do
			run_one_workload_omnicache "ycsb-${job}"
		done
	else
		run_one_workload_omnicache "$1"
	fi
fi