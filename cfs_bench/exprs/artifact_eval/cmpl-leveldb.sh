...
if [ ! "$1" = "ufs" ] && [ ! "$1" = "ext4" ] && [ ! "$1" = "oxbow" ] && [ ! "$1" = "omnicache" ]; then print_usage_and_exit; fi
...
if [ "$1" = "ufs" ]; then
	...
	LDB_CMAKE_CFS_ARG="-DLEVELDB_JL_LIBCFS=OFF -DLEVELDB_JL_OXBOW=OFF -DLEVELDB_JL_OMNICACHE=ON"
else
	echo "Unknown filesystem: $1"
