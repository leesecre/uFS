...
if [ ! "$1" = "all" ]; then print_usage_and_exit; fi
...
cd "$AE_BENCH_REPO_DIR/FS_microbench"

# Create directory for data
data_dir="$(mk-data-dir fs_micro_$1_$2)"
...
if [ "$2" = "ufs" ]; then
	...
	if [ "$1" = "all" ]; then
		sudo -E python3 scripts/run_fs_micro_all_ufs.py "$data_dir"
	fi
	...
	# Run benchmark
	if [ "$1" = "all" ]; then
		sudo -E python3 scripts/run_fs_micro_all_ext4.py "$data_dir"
	fi
