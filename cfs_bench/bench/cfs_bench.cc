...
private:
  ...
  void DoWrite(ThreadState *thread, bool seq) {
    ...
    // Pre-compute non-overlapping offsets for random writes when requested.
    std::vector<off_t> off_vec;
    uint64_t max_req_num = numop_;
    if (!seq && FLAGS_rand_no_overlap) {
      if (value_random_size_) {
        fprintf(stderr,
                "rand_no_overlap=1 requires value_random_size=0 for writes\n");
        exit(1);
      }
      if (FLAGS_rw_align_bytes == 0) {
        fprintf(stderr,
                "random_no_overalap=1 => rw_align_bytes should not be 0\n");
        exit(1);
      }
      if (value_size_ > (int)FLAGS_rw_align_bytes) {
        fprintf(stderr,
                "rand_no_overlap=1 requires value_size (%d) ≤ rw_align_bytes "
                "(%d) to avoid overlap; increase rw_align_bytes or reduce "
                "value_size.\n",
      ...
      if (FLAGS_block_no >= 0) {
        fprintf(stderr,
                "rand_no_overlap=1 conflicts with block_no; unset block_no\n");
        exit(1);
      }
      if (randWriteRange == 0) {
        fprintf(stderr,
                "random write range is 0; file too small or not initialized\n");
        exit(1);
      }
      ...
      if (numop_ > max_req_num) {
        fprintf(stderr,
                "Insufficient write range for no-overlap random write: need "
                "%d ops * %uB stride, have only %lu slots (range=%lu). "
                "Increase file size/max_file_size or reduce numop.\n",
                numop_, (unsigned)FLAGS_rw_align_bytes, max_req_num,
                (unsigned long)randWriteRange);
        exit(1);
      }
    ...
    for (int i = 0; i < numop_; i++) {
      ...
      if (seq) {
        ...
        if (FLAGS_rand_no_overlap && FLAGS_rw_align_bytes != 0) {
          ...
          fprintf(stderr, "[cfs_bench] rand cur_off:%ld\n", cur_off);
          if (FLAGS_rw_align_bytes != 0) {
  ...
  void DoRead(ThreadState *thread, bool seq) {
    ...
    if (FLAGS_rand_no_overlap) {
      ...
      if (numop_ > max_req_num) {
        fprintf(stderr,
                "Insufficient file size for no-overlap random read: need %d "
                "ops * %uB stride, have only %lu slots (fileSize=%lu). "
                "Increase file size or reduce numop.\n",
                numop_, (unsigned)FLAGS_rw_align_bytes, max_req_num,
                (unsigned long)thread->fileSize);
        exit(1);
      }
    } else {
